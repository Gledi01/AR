...
CORE_PKGS=(openbox picom rofi lxappearance papirus-icon-theme feh xterm)

# nitrogen: sudah di-drop dari Debian testing sejak akhir 2025, jadi kemungkinan besar
# tidak punya installation candidate di Kali rolling. feh dipakai sebagai wallpaper
# setter utama (selalu tersedia); nitrogen hanya dipasang best-effort kalau repo
# Anda kebetulan masih menyediakannya (mis. mirror belum sinkron / repo pihak ketiga).
if pkg_installed nitrogen; then
    CORE_PKGS+=("nitrogen")
elif pkg_available nitrogen; then
    CORE_PKGS+=("nitrogen")
    log_info "nitrogen terdeteksi tersedia di repo Anda, akan dicoba diinstall sebagai bonus."
else
    log_info "nitrogen tidak tersedia di repo (paket ini sudah di-drop dari Debian testing/Kali rolling). Memakai feh sebagai wallpaper setter."
fi
...
if [ "${#TO_INSTALL[@]}" -gt 0 ]; then
    ...
    if sudo apt-get install -y "${TO_INSTALL[@]}"; then
        ...
        log_warn "Instalasi batch gagal (kemungkinan satu paket bermasalah/cache basi membuat"
        log_warn "apt-get mengira tersedia padahal tidak ada kandidat). Mencoba satu per satu..."
        FAILED_PKGS=()
        for p in "${TO_INSTALL[@]}"; do
            if sudo apt-get install -y "$p"; then
                log_ok "Terinstall: $p"
            else
                log_warn "Gagal install, dilewati: $p"
                FAILED_PKGS+=("$p")
            fi
        done
        if [ "${#FAILED_PKGS[@]}" -gt 0 ]; then
            log_warn "Paket yang gagal (dilewati, tidak menghentikan script): ${FAILED_PKGS[*]}"
        fi
        log_ok "Instalasi paket selesai (mode satu per satu)."
    fi
else
...
if cmd_exists nitrogen; then
    add_sys_item "Wallpaper" "nitrogen"
elif [ -n "$FILEMANAGER_APP" ]; then
    add_sys_item "Wallpaper Folder" "${FILEMANAGER_APP} $HOME/Pictures/wallpapers"
fi
...
if [ -n "$WALLPAPER_FILE" ]; then
    ...
    log_warn "lalu jalankan ulang autostart (atau 'openbox --restart'). Sebagai fallback, warna"
    log_warn "latar solid dark akan dipakai lewat xsetroot."
fi

if cmd_exists nitrogen; then
    log_info "nitrogen terdeteksi terinstall, akan dipakai sebagai wallpaper manager (dengan feh sebagai cadangan)."
